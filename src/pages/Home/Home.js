    ...
    <>
      ...
      <header className="relative w-100vw">
        ...
        <div className="center-div-method-2 header-text">
          <h1 className="w-100vw text-align-center">
            Get everything on F1 at one place
          </h1>
          <br />
          <div
            className="m-auto flex-center-center shop-now-btn"
            href="/Pages/product.html"
          >
            <Link
              to={"/explore"}
              className="bg-accent p-2 mt-2 flex-center-center "
              id="explore-link"
            >
              Explore{" "}
              <span className="material-icons icon-s3">arrow_forward_ios</span>
            </Link>
          </div>
        </div>
      </header>
      <main className="w-100vw flex-center-center flex-col">
        <div className="catagory w-100per flex-center-center flex-wrap p-2">
          {categories.map(({ categoryName, _id }) => (
            <button onClick={e=>categoryCardClickHandler(categoryName)} key={_id}>
